...
Convierte archivos de audio .aac / .m4a (sin video) a .mp3 en alta calidad,
usando ffmpeg.
...
USO POR LÍNEA DE COMANDOS (sin ventana):
    python audio_a_mp3.py "C:\\ruta\\a\\mi\\carpeta" --recursivo

OPCIONES (modo línea de comandos):
    --recursivo       Busca .aac/.m4a también en subcarpetas.
...
def convertir_a_mp3(archivo: Path, carpeta_salida: Path, calidad: int, log=print):
    ...
    cmd = [
        ...
        "-vn",                       # por si acaso trae carátula/imagen embebida
        "-codec:a", "libmp3lame",
...
def buscar_archivos(carpeta: Path, recursivo: bool):
    patrones = ["**/*" + ext for ext in EXTENSIONES_ORIGEN] if recursivo else ["*" + ext for ext in EXTENSIONES_ORIGEN]
    archivos = []
...
def procesar_carpeta(carpeta: Path, salida: Path, recursivo: bool, calidad: int, log=print):
    archivos = buscar_archivos(carpeta, recursivo)

    if not archivos:
        log(f"No se encontraron archivos .aac/.m4a en: {carpeta}")
        return 0, 0

    log(f"Encontrados {len(archivos)} archivo(s) de audio. Convirtiendo a mp3...\n")
    exitos = 0
...
def modo_gui():
    ...
    root = tk.Tk()
    root.title("Convertir AAC/M4A a MP3")
    ...
    def elegir_carpeta():
        carpeta = filedialog.askdirectory(title="Elegí la carpeta con los audios .aac/.m4a")
        if carpeta:
    ...
    def iniciar_conversion():
        if not carpeta_elegida["ruta"]:
            messagebox.showwarning("Falta carpeta", "Primero elegí una carpeta con audios.")
            return
        ...
        if total == 0:
            messagebox.showinfo("Sin archivos", "No se encontraron .aac/.m4a en esa carpeta.")
        else:
            messagebox.showinfo("Listo", f"{exitos}/{total} archivos convertidos.\nGuardado en:\n{salida}")

    tk.Label(root, text="Convertir AAC/M4A a MP3", font=("Segoe UI", 14, "bold")).pack(pady=(15, 5))
    tk.Button(root, text="Elegir carpeta con audios...", command=elegir_carpeta).pack(pady=5)
...
def main():
    parser = argparse.ArgumentParser(description="Convierte archivos .aac/.m4a a .mp3.")
    parser.add_argument("carpeta", nargs="?", help="Carpeta con los archivos .aac/.m4a (si se omite, abre ventana)")
